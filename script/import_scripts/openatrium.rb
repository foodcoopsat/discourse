...
class ImportScripts::OpenAtrium < ImportScripts::Base
  ...
  def import_users
    ...
    results = @client.query("
          SELECT u.uid id, u.pass pass, u.name username, u.mail email, u.created created,
                 u.access access, u.status status, n.title name, p.field_profile_url_value url
            FROM users u
       LEFT JOIN profile_values pv
              ON pv.uid = u.uid
       LEFT JOIN node n
              ON n.type = 'profile' AND n.uid = u.uid
       LEFT JOIN content_type_profile p
              ON p.nid = n.nid
           WHERE u.uid = 0
              OR u.status = 1
              OR u.uid IN (SELECT uid FROM comments UNION SELECT uid FROM node)
        ")

    create_users(results) do |row|
      if row['status'] != '1'
        suspended_at = Time.now
        suspended_till = Time.now + 500.years
      end

      {
        ...
        last_seen_at: Time.zone.at(row['access'])
      }
    end
  end
    ...
    results = @client.query("
       ...
       LEFT JOIN content_type_event cte
           ...
           WHERE n.type IN ('blog', 'book', 'event')
             AND n.nid = nr.nid
             AND n.nid = oa.nid
        GROUP BY nr.nid, oa.group_nid, nr.uid, nr.title, nr.body, nr.timestamp, t.tags, u.cnt
        ", cache_rows: false)
    ...
    create_posts(results) do |row|
      {
        ...
        raw: add_vid_attachments_to_body(row['body'], row['uploads'], row['vid']),
        tags: row['tags'].split(','),
        created_at: Time.zone.at(row['created'])
      }
    end
  ...
  def import_event_details
    puts '', 'adding event details to topics'

    results = @client.query("
        SELECT cfd.nid nid, cfd.field_date_value start_date, cfd.field_date_value2 end_date
          FROM content_field_date cfd,
               (  SELECT nid, MAX(vid) vid
                    FROM node_revisions
                GROUP BY nid) nr
          WHERE cfd.nid = nr.nid
            AND cfd.vid = nr.vid
       ORDER BY cfd.nid ASC
      ", cache_rows: false)

    current = 0
    results.each do |row|
      if topic_mapping = topic_lookup_from_imported_post_id("nid:#{row['nid']}")
        topic  = Topic.find(topic_mapping[:topic_id])
        topic.custom_fields['event_start'] = Time.zone.parse(row['start_date']).to_i
        topic.custom_fields['event_end'] = Time.zone.parse(row['end_date']).to_i
        topic.save!
      else
        puts "No topic found for node #{row['nid']}"
      end
    ...
    batches(batch_size) do |offset|
      results = @client.query("
          SELECT c.cid cid, c.pid pid, c.nid nid, c.uid uid, c.comment body, c.timestamp created,
            ...
            FROM og_ancestry oa, node n, comments c
